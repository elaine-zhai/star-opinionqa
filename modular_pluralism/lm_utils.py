...
def answer_parsing(response):
    # Match pattern like: Final Answer: B or Final Answer: (B)
    match = re.search(r"Final Answer:\s*\(?([A-E])\)?", response, re.IGNORECASE)
    if match:
        return match.group(1).upper()
    return "Z"  # Clearly wrong, ensures model isn't rewarded for a bad parse
