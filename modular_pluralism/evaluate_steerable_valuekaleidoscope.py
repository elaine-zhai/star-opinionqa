...
if __name__ == "__main__":
    ...
    pred = []
    gold = []
    option_to_number_map = {"A": 0, "B": 1, "C": 2, "D": 3}
    for item in tqdm(data):
        gold.append(option_to_number_map[item["label"]]) #label 
        #print("RAW OUTPUT:", item["output"])
        now_pred = answer_parsing(item["output"]) #output
        #print("PARSED LABEL:", now_pred)
        if now_pred in option_to_number_map:
            ...
            if "support" in item["output"].lower():
                ...
                pred.append(1)
            elif "either" in item["output"].lower() or "it depends" in item["cot"].lower():
                pred.append(2)
            else:
